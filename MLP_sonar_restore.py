...
with tf.Session() as sess:
  ...
  for i in range(93, 101):
    prediction_run = sess.run(prediction, feed_dict={x: train_x[i].reshape(1, 60)})
    accuracy_run = sess.run(accuracy, feed_dict={x: train_x[i].reshape(1, 60), y: train_y[i].reshape(1, 2)})
    y1 = sess.run(tf.argmax(train_y, 1))
